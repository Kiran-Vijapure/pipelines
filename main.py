from fastapi import FastAPI, Request, Depends, status, HTTPException, UploadFile, File, BackgroundTasks
...
# async def generate_openai_chat_completion(form_data: OpenAIChatCompletionForm, bg_task: BackgroundTasks):
async def generate_openai_chat_completion(form_data: OpenAIChatCompletionForm):
    ...
    def job():
        ...
        if pipeline["type"] == "manifold":
            ...
            pipe_obj = PIPELINE_MODULES[manifold_id]
            pipe = PIPELINE_MODULES[manifold_id].pipe
            mlflow_func = PIPELINE_MODULES[manifold_id].mlflow_logging
        else:
            pipe_obj = PIPELINE_MODULES[pipeline_id]
            pipe = PIPELINE_MODULES[pipeline_id].pipe
            mlflow_func = PIPELINE_MODULES[pipeline_id].mlflow_logging

        if form_data.stream:

            def stream_content():
                answer = ""
                res, ref_list, nodes = pipe(
                ...
                if isinstance(res, str) or isinstance(res, Generator):
                    if pipe_obj.reference_str:
                        line = stream_message_template(form_data.model, pipe_obj.reference_str)
                        yield f"data: {json.dumps(line)}\n\n"
                        # bg_task.add_task(mlflow_func, user_message, answer, ref_list, nodes)
                        mlflow_func(user_message, answer, ref_list, nodes)
